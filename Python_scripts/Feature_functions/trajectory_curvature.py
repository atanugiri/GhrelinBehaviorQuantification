...
def _get_frame_rate(conn: PGConnection, trial_id: int) -> float:
    """Fetch frame_rate metadata for a trial."""
    q = "SELECT frame_rate FROM dlc_table WHERE id = %s;"
    df = pd.read_sql_query(q, conn, params=(trial_id,))
    if df.empty or pd.isna(df.loc[0, "frame_rate"]):
        raise ValueError(f"frame_rate not found for id={trial_id}")
    return float(df.loc[0, "frame_rate"])
...
def compute_trajectory_curvature(conn: PGConnection,
    ...
    """
    # 1) Load trajectory
    x_vals, y_vals = get_normalized_bodypart(trial_id, conn, bodypart, normalize=True, interpolate=True)
